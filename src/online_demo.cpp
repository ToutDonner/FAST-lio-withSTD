...
std::string oriTraj_path, optTraj_path;

std::vector<PointCloud::Ptr> cloud_vec;       // save pointcloud body
std::vector<Eigen::Affine3d> pose_vec;        // optimizated pose
std::vector<Eigen::Affine3d> origin_pose_vec; // origin pose
...
bool syncPackages(PointCloud::Ptr &cloud, Eigen::Affine3d &pose, double &time)
{
...
void SaveTrajOpt(const std::string &traj_file)
{
  ...
  assert(time_vec.size() == pose_vec.size());

  ofs << "#timestamp x y z q_x q_y q_z q_w" << std::endl;
  for (int i = 0; i < pose_vec.size(); i++)
  {
    Eigen::Quaterniond r_(pose_vec[i].rotation());

    ofs << std::fixed << std::setprecision(6) << time_vec[i] << " " << std::setprecision(15)
        << pose_vec[i].translation().x() << " " << pose_vec[i].translation().y() << " " << pose_vec[i].translation().z() << " " << r_.x()
        << " " << r_.y() << " " << r_.z() << " " << r_.w() << std::endl;
  }
...
void SaveTrajOri(const std::string &traj_file)
{
  ...
  assert(time_vec.size() == origin_pose_vec.size());

  ofs << "#timestamp x y z q_x q_y q_z q_w" << std::endl;
  for (int i = 0; i < origin_pose_vec.size(); i++)
  {
    Eigen::Quaterniond r(origin_pose_vec[i].rotation());
    ofs << std::fixed << std::setprecision(6) << time_vec[i] << " " << std::setprecision(15)
        << origin_pose_vec[i].translation().x() << " " << origin_pose_vec[i].translation().y() << " " << origin_pose_vec[i].translation().z() << " " << r.x()
        << " " << r.y() << " " << r.z() << " " << r.w() << std::endl;
  }
...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    Eigen::Affine3d pose;
    double pose_time;
    if (syncPackages(current_cloud_body, pose, pose_time))
    {
  ...
  pcl::io::savePCDFileBinary("/home/crz/Algorithm/FAST-LIO/src/FAST-lio-withSTD/PCD/optscans.pcd", full_map);
