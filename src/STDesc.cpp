...
void read_parameters(ros::NodeHandle &nh, ConfigSetting &config_setting)
{
  ...
  // pre-preocess
  nh.param<double>("STD/ds_size", config_setting.ds_size_, 0.5);
  nh.param<int>("STD/maximum_corner_num", config_setting.maximum_corner_num_, 100);

  // key points
  nh.param<double>("vplane_merge_normal_thre",
                   config_setting.plane_merge_normal_thre_, 0.1);
  nh.param<double>("STD/plane_detection_thre", config_setting.plane_detection_thre_,
                   0.01);
  ...
  nh.param<double>("STD/proj_image_resolution",
                   config_setting.proj_image_resolution_, 0.5);
  nh.param<double>("STD/proj_dis_min", config_setting.proj_dis_min_, 0);
  nh.param<double>("STD/proj_dis_max", config_setting.proj_dis_max_, 2);
  nh.param<double>("STD/corner_thre", config_setting.corner_thre_, 10);

  // std descriptor
  nh.param<int>("STD/descriptor_near_num", config_setting.descriptor_near_num_, 10);
  nh.param<double>("STD/descriptor_min_len", config_setting.descriptor_min_len_, 2);
  nh.param<double>("STD/descriptor_max_len", config_setting.descriptor_max_len_,
                   50);
  nh.param<double>("STD/non_max_suppression_radius",
                   config_setting.non_max_suppression_radius_, 2.0);
  nh.param<double>("STD/std_side_resolution", config_setting.std_side_resolution_,
                   0.2);

  // candidate search
  nh.param<int>("STD/skip_near_num", config_setting.skip_near_num_, 50);
  nh.param<int>("STD/candidate_num", config_setting.candidate_num_, 50);
  nh.param<int>("STD/sub_frame_num", config_setting.sub_frame_num_, 10);
  nh.param<double>("STD/rough_dis_threshold", config_setting.rough_dis_threshold_,
                   0.01);
  nh.param<double>("STD/vertex_diff_threshold",
                   config_setting.vertex_diff_threshold_, 0.5);
  nh.param<double>("STD/icp_threshold", config_setting.icp_threshold_, 0.5);
    ...
    std::unordered_map<VOXEL_LOC, OctoTree *> &voxel_map)
{
  ...
  for (uint i = 0; i < plsize; i++)
  {
    ...
    /**
     * @brief 用down sample之后的位置作为hash map的索引
     *
     * @return VOXEL_LOC
     */
    VOXEL_LOC position((int64_t)loc_xyz[0], (int64_t)loc_xyz[1],
